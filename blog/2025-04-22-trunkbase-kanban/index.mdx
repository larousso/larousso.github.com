...
# Trunk base sous steroids et kanban

À la MAIF, le flow de dév le plus communément appliqué, c'est du gitflow et du srcum.
Personnellement, j'ai du mal à comprendre le gitflow et je ne comprends pas à quoi sert la branche develop du moins pour le développement d'application.
...
Tous les lundis, on fait le point sur les features qui ont été développées et testées et qui sont prêtes à partir.
...
Comme indiqué plus tôt, les tests automatisés présents dans nos livrables ne suffisent pas. Notre testeur à également une batterie de test automatisés qui s'appuient sur un environnement de QA, ça fait un bon filet de sécurité en plus.
