...
Le sharding est un algo très utilisé dans le monde des systèmes distribué. C'est un algo assez qui peut aussi vous aidez dans vos développements de tous les jours.
...
Le cas d'école est le suivant : on a identifiants qu'on souhaite répartir vers des noeuds d'un cluster (ou bien des threads ou n'importe quoi).
Ce qu'on veut, c'est
* Pouvoir répartir uniformément les identifiants entre les noeuds du cluster
* S'assurer qu'un même identifiant ira toujours sur le même noeud
...
* le modulo du hash : hash % le nombre de noeud = le numéro du noeud vers lequel router l'id

Pour un id `30d62f77-bb54-4b9a-962f-77bb541b9ae1`, si on a 10 noeuds et que :

`hash(id) % 10 = 5`

Alors on enverra l'id sur le noeud numéro 5.

Il reste maintenant à trouver un algo de hachage qui, a une bonne répartition quelle que soit la chaîne de caractère en entrée. Et à priori c'est `MurmurHash3` qui semble faire l'unanimité.
...
Résultat : on a environ 1000 éléments par channel, c'est pas trop mal
...
## Qui on utilise cet algorithme ?

Pas mal de systèmes distribués utilisent cet algorithme, ou bien une version amélioré.
...
Akka cluster utilise ça avec son module de cluster sharding.

## Comment on utilise cet algorithme ?
...
Ici, on commence à lire le fichier, on parallélise par 2, on fait des paquets de 2 pour faire des bulks updates à l'API
...
On voit vite le problème, les données avec les mêmes ids sont traités en parallèle, on va avoir des modifications concurrentes et donc on risque des perdre des mises à jour.

J'ai une idée ! Et si on utilisait le sharding !

Comme on parallélise par 2, on va faire :
`hash(id) % 2` pour savoir si on envoit l'id dans le channel 0 ou dans le channel 1. Attention, au moment des batchs updates, il faudra grouper les modifs d'un même id
...
#### Répartir des jobs sur les noeuds d'un cluster

On veut scheduler des jobs sur un cluster avec un nombre de noeuds fixes, on génère un id pour les job et c'est parti !

#### Faire de l'AB testing

On veut présenter la feature A à 20% des utilisateurs, et la feature B aux 80% restant, par de problème :

si `hash(id) % 100 < 20` c'est A sinon c'est B. Comme le hash(id) aura toujours la même valeur, l'utilisateur aura toujours la même version.
Pas besoin de stocker d'état en base, c'est super simple !

## Conclusion

C'est un algo que j'utilise régulièrement quand je dois traiter de la donnée, il m'a aidé dans un paquet de situations, j'espère qu'il vous rendra service aussi un des ces quatres !
