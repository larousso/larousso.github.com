...
Dans le post précédent, on a vu les fondations de la programmation fonctionnelle, ici, on va s'intéresser plus en détail à l'immutabilité en java.
...
Il existe des outils dans le jdk pour gérer l'immutabilité mais,
malheureusement, comme souvent avec java, il y a les fondations, mais on doit rapidement ajouter des dépendances pour profiter d'API plus fluide.

Manipuler en permanences des objets immutables a un coût d'apprentissage et, ça peut être un peu rugueux pour des développeurs qui sont habitués à du java "classique" mutable.

Une fois le coup de main pris, manipuler des objets immutable libère un peu la tête, pas de mauvaises surprises, pas de risques que l'objet ait été modifié hors de la méthode courante.
