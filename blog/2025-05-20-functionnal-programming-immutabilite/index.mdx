...
Avec des langages un peu plus récents comme kotlin ou scala, ou l'immutabilité a été pris en compte dès le départ, on aura une syntaxe simplifiée avec les mots clés `var` et `val` :
...
En utilisant des structures avec des attributs immutables, on peut vite se rendre compte que ça devient assez difficile de modifier la feuille d'une structure un peu plus complèxe :
...
Nous venons de voir comment créer nos propres structures immutables, mais que propose java en terme d'immutabilité dans le jdk ?
...
Pourtant, certaines classes sont immutables par défaut, et depuis très longtemps, on peut penser à `java.lang.String`.
...
Tout le monde a bien intégré que `String` est immutable et que la concatenation produira une nouvelle valeur :
...
Comme pour les structures immutables, l'immutabilité dans les collections vient avec son lot de manipulations un peu tricky.
...
Manipuler en permanences des objets immutables aura un coût d'apprentissage et il faudra probablement faire preuve d'ingéniosité pour résoudre certaines situations.

Une fois le coup de main pris, manipuler des objets immutables libère un peu la tête, pas de mauvaises surprises, pas de risques que l'objet ait été modifié hors de la méthode courante.
