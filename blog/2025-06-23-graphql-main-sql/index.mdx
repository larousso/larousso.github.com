...
flowchart LR
    ...
    Traductions et titres alternatifs
    ]
...
Jooq propose plusieurs alternatives, on peut aller d'un builder de query typesafe à des requêtes sous forme de texte, voir mixer tout ça ensemble.
...
Cette approche fonctionne très bien, mais, elle a un inconvénient : on fait une requête à la base par personne et par champ.

Ceci peut avoir des impacts d'un point de vue temps de réponse et sollicitation à la base.

### Utiliser l'annotation `@BatchMapping`

L'annotation `@BatchMapping`, est un shortcut vers la notion de `BatchLoader` présent dans la lib `graphql-java` sur laquelle s'appuie spring.
...
Les features JSON de posgtresql sont très pratiques pour ce genre de use-case et permettent de très simplement de gérer les aspects de pagination, filtres, etc.
