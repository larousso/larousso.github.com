---
slug: functionnal-programming-own-monad
title: Prog fonct en java - écrire sa monade
authors: [adelegue]
tags: [patterns, java, prog-fonctionnelle]
---

Cette fois, on va aller encore un peu plus loins : on va écrire notre propre monade !
...
Si vous faites du spring reactif, ça peut vite devenir la plaie de gérer du métier.

Si on souhaite avoir des erreurs explicites, on peut utiliser un `Either` mais le problème, c'est qu'on se retrouve avec des `Mono<Either<E, A>>` en permanence et, ça alourdit énormément le code.

Pour manipuler cette structure, il faut sans cesse manipuler 2 niveaux : le `Mono` puis le `Either` :
...
Comme on est gourmand, on va ajouter une autre capacité à notre monade, la capacité de mettre de côté des événements à publier. Comme on est encore plus gourmand, on va se passe de vavr et écrire ça en java vanilla !
...
Tout d'abord, on commence par l'either. Depuis le jdk 21, on peut utiliser les `sealed` interface pour créer des types "union". On va pouvoir écrire l'either ainsi :
...
Il faut ensuite qu'on écrive une structure qui permet de stacker nos événements en plus de la valuer courante. Il existe une monade qui permet de faire ça, c'est la writer monad.
...
La règle à respecter est `m.flatMap(unit) == m`. Comme pour l'identité à gauche, on va aussi tester les cas ou notre `IO` termine en erreur.
...
Ici, on va combiner les succès, mais aussi les erreurs. Si ça se passe mal, on voudrait pouvoir récupérer tous les problèmes d'un coup :

```java
public static <S, E, A, B, C> IO<S, List<E>, C> product(IO<S, E, A> ioA, IO<S, E, B> ioB, BiFunction<A, B, C> combinator) {

    // On définie un tuple local pour le switch
...
Si vous êtes arrivé jusqu'ici et que vous êtes à l'aise avec tous les concepts abordés, je vous donne votre ceinture noire de fonctionnel : c'est le début officiel de votre apprentissage en prog fonctionelle.
...
Si vous trouvez tout ça overkill, vous pouvez seulement conserver certains aspects. Quelques exemples :

* immutabilité à l'exterieur des méthodes, mais mutabilité dans le body des méthodes
